...
35. **AI Chatbot + Owner API Key Management** — Migration `20260516000001`: tabel `site_settings` (gemini_api_key, owner-only RLS) + `rate_limits` (IP tracking). Edge Function `ai-assistant`: static response router (3 predefined Q&A bypass API), `npm:@google/genai` SDK, model `gemini-2.5-flash`, system instruction konteks Iku Gadget, IP rate limit 15/jam, service role DB access. `ChatAssistant.tsx`: FAB bottom-right, Sheet sidebar, greeting + quick topics, message history, error display. `AiSettings.tsx`: Owner form Gemini API key. Logo responsive.

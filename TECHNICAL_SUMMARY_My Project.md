...
27. **Testimonial Moderation + COD Business Model** — `TestimonialModeration.tsx`: tombol "Sembunyikan" (revert ke pending) dan "Hapus" (delete permanen) untuk ulasan live. `LandingPage.tsx`: copywriting diubah ke model COD online — Cara Kerja (Chat WA → Nego Online → COD), "Fast Response 6 Hari", CTA "atur jadwal COD", alamat fisik dihapus → "Melayani COD area Surabaya & sekitarnya".

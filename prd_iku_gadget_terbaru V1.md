...
| 14 Mei 2026 | 2.2.0 | (1) TestimonialModeration: tombol "Sembunyikan" (set status→pending) dan "Hapus" (delete permanen) ditambahkan ke ulasan yang sudah dipublikasikan. (2) Landing Page copywriting diubah ke model bisnis COD online: Hero sub-headline menyebut "online + COD Surabaya", Cara Kerja 3 step diubah (Chat WA → Nego Online → COD), feature card "Buka 6 Hari" → "Fast Response 6 Hari", CTA "atur jadwal COD", alamat fisik dihapus diganti "Melayani COD area Surabaya & sekitarnya", footer tagline "Jual Beli HP Bekas Online — COD Surabaya". Semua referensi toko fisik dieliminasi. | `src/components/admin/TestimonialModeration.tsx`, `src/pages/LandingPage.tsx` |
